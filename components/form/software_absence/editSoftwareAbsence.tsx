...
import { AbsenceEditSchema } from '@/src/helpers/definition'
...
import { editSoftwareAbsence } from "@/src/features/actions/software_absence/software_absence.actions";
...
import type { getSoftwareAbsenceBySlug } from "@/src/query/software_absence.query"
export default function EditSoftwareAbsence({ clientSlug, softwareSlug, absenceSlug, methodOfCalcul, counter, dsnCode, absence }: { clientSlug: string, softwareSlug: string, absenceSlug: string, methodOfCalcul: getParamByIdAndSoftwareActive, counter: getCounterForMyActiveSoftware, dsnCode: getDsnAbsence, absence: getSoftwareAbsenceBySlug }) {
    const [loading, setLoading] = useState(false)

    const form = useForm<z.infer<typeof AbsenceEditSchema>>({
        resolver: zodResolver(AbsenceEditSchema),
        defaultValues: {
            id: absence.id,
            clientSlug: clientSlug,
            absenceSlug: absenceSlug,
            softwareSlug: softwareSlug,
            methodOfCalcul: absence.methodOfCalcul,
            dsnCode: absence.dsnCode ?? '',
            label: absence.label,
            description: absence.description ?? '',
            counter: absence.counter ?? '',
            population: absence.population ?? '',
            isSocialSecurity: absence.isSocialSecurity,
            isPrintable: absence.isPrintable,

        },
    })
    const onSubmit = async (data: z.infer<typeof AbsenceEditSchema>) => {
        try {
            setLoading(true)
            const action = await editSoftwareAbsence(data)
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input type="text" placeholder="" disabled {...field} required />
                            </FormControl>
                        </FormItem>
                    )}
                />
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                ...
                                <SelectContent>
                                    {methodOfCalcul.map((method) => (<SelectItem key={method.label} value={method.label}>{method.value}</SelectItem>))}
                                </SelectContent>
                            </Select>
                            <FormMessage />
                        </FormItem>
