...
export default async function Page({ params }: { params: { clientSlug: string, softwareSlug: string } }) {
    ...
    const user = await security.isEditorClient(clientExist.siren)
    if (!user) {
    ...
    const { countForm, countItems, countPage, countSetting } = await software.getCountSoftwareElement()
                            ...
                            >
                                <CardHeader className="pb-3">
                                    ...
                                    <CardDescription className="max-w-lg text-balance leading-relaxed">
                                        Commencer à éditer les données de votre logiciel <span className='font-bold'>{softwareExist?.label}</span>
                                    </CardDescription>
                                </CardHeader>
                                <CardFooter>
                                ...
                                <Card x-chunk="dashboard-05-chunk-3">
                                    ...
                                    <CardContent>
                                        <Table>
                                            ...
                                            <TableBody>
                                                <TableRow>
                                                    <TableCell className="font-medium">Liste déroulante</TableCell>
                                                    <TableCell>{countSetting}</TableCell>
                                                    <TableCell className="text-right"><Link href={`/client/${client.clientSlug}/editor/${params.softwareSlug}/setting`}><ArrowRight /></Link></TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell className="font-medium">Pages</TableCell>
                                                    <TableCell>{countPage}</TableCell>
                                                    <TableCell className="text-right"><Link href={`/client/${client.clientSlug}/editor/${params.softwareSlug}/page`}><ArrowRight /></Link></TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell className="font-medium">Formulaire</TableCell>
                                                    <TableCell>{countForm}</TableCell>
                                                    <TableCell className="text-right"><Link href={`/client/${client.clientSlug}/editor/${params.softwareSlug}/form`}><ArrowRight /></Link></TableCell>
                                                </TableRow>
                                                {refs?.Software_Setting_Value.map((ref) => {
                                                    return (
                                                        <TableRow key={ref.id}>
                                                            <TableCell className="font-medium">{ref.label}</TableCell>
                                                            <TableCell>10</TableCell>
                                                            <TableCell className="text-right"><Link href={`/client/${client.clientSlug}/editor/${params.softwareSlug}/dynamic/${ref.slug}`}><ArrowRight /></Link></TableCell>
                                                        </TableRow>
                                                    )
                                                })}
