...
import { editSoftwareItem } from "@/src/features/actions/softwareItems/softwareItems.actions"
import {
    Form,
    FormControl,
    FormDescription,
...
export default function Edit({ item, softwares, idccList }: { item: getSoftwareItemsBySlugType, softwares: getSoftwareByUserIsEditorType[], idccList: getIdccByCodeType[] }) {
    if (!item) {
        throw new Error('Software item slug is required')
    }
    const editSoftwareItemWithId = editSoftwareItem.bind(null, item.slug)
    ...
    const onSubmit = async (data: z.infer<typeof SoftwareItemSchema>) => {
        try {
            ...
            await editSoftwareItemWithId(data)
        } catch (err) {
            console.log(err)
